...
from PySide6.QtWidgets import (
    QGroupBox, QHBoxLayout, QLabel, QListWidget, QPlainTextEdit, QPushButton, QVBoxLayout, QWidget
)
...
class BlenderPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.request.setPlaceholderText("예: geometry_0을 1.05배 확대하고 smooth shading을 적용해줘")
        self.request.setMaximumHeight(100)
        layout.addWidget(self.request)
        self.propose = QPushButton("실행 계획 만들기")
        self.propose.clicked.connect(lambda: self.propose_requested.emit(self.request.toPlainText()))
        ...
        self.versions.itemDoubleClicked.connect(lambda item: self.open_file_requested.emit(item.data(256)))
    ...
    def set_inspection(self, payload: dict) -> None:
        ...
        for item in data.get("objects", []):
            lines.append(f"- {item.get('name')} ({item.get('type')}), dimensions={item.get('dimensions')}, materials={item.get('material_slots')}")
    ...
    def set_job(self, job, busy: bool = False) -> None:
        ...
        for artifact in job.artifacts:
            if artifact.stage == "blender":
                self.versions.addItem(f"v{artifact.version:03d} · {artifact.kind.upper()} · {Path(artifact.path).name}")
