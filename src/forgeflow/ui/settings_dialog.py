...
def validate_server_url(value: str) -> str:
    ...
    try:
        ...
        valid = False
    if not valid:
        raise ValueError("Ollama 주소는 http://127.0.0.1:11434처럼 입력하세요. 포트는 1~65535이며 로그인 정보·쿼리·공백은 사용할 수 없습니다.")
    return value
...
class SettingsDialog(QDialog):
    ...
    def browse_path(self, key: str) -> None:
        ...
        if key == "blender_executable":
            selected, _ = QFileDialog.getOpenFileName(self, "Blender 실행 파일 선택", current, "실행 파일 (*.exe)")
        else:
    ...
    def refresh_models(self) -> None:
        ...
        endpoints = {}
        for key, address in (("ollama_model", self.edits["ollama_base_url"].text()), ("unity_agent_model", unity_url)):
        ...
        for address, keys in endpoints.items():
            ...
            reply.finished.connect(lambda response=reply, fields=keys, endpoint=address: self._models_ready(response, fields, endpoint, generation))
    ...
    def _models_ready(self, reply, keys, address, generation) -> None:
        ...
        try:
            ...
            names = sorted({item["name"] for item in models if isinstance(item, dict) and isinstance(item.get("name"), str)})
            for key in keys:
                combo = self.model_combos[key]
                current = combo.currentText()
                combo.clear()
            ...
            for key in keys:
                self.model_status[key].setText(f"목록 조회 실패 · {address}\n{exc}\n모델명을 직접 입력해 저장할 수 있습니다.")
        finally:
            reply.deleteLater()
