...
def run_command(command, log_path: Path) -> int:
    lines: list[str] = []
    ...
    code = SyncProcessRunner().run(command, receive, timeout=1800)
...
def main() -> int:
    ...
    meshes = [item for item in before.get("data", {}).get("objects", []) if item.get("type") == "MESH"]
    if not meshes:
    ...
    jobs.save(job)
    print(json.dumps({"type": "e2e_completed", "job_directory": str(root), "evidence": str(evidence_path)}, ensure_ascii=False))
    return 0
