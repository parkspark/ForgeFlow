...
FBX_PROMPT = (
    "이 Humanoid FBX를 이용해 새 테스트 씬에 캐릭터를 배치하고 카메라에서 전체 모습이 보이게 해줘."
)
...
def main() -> int:
    ...
    evidence_name = (
        "unity-e2e-evidence.json" if args.only == "all" else f"unity-e2e-evidence-{args.only}.json"
    )
            ...
            evidence["runs"].append(
                {
                    ...
                    "milestones": [
                        event for event in events[before:] if event.get("type") == "milestone"
                    ],
