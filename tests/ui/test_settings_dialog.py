...
from forgeflow.ui.dialogs.settings_dialog import SettingsDialog, validate_server_url
...
def valid_config(config, tmp_path):
    ...
    return replace(
        config, unity_agent_root=config.blender_agent_root, unity_mcp_root=config.blender_mcp_root
    )
...
@pytest.mark.parametrize(
    "address", ["http://127.0.0.1:11434", "https://example.com/ollama", "http://[::1]:11434"]
)
...
@pytest.mark.parametrize(
    ...
    [
        ("jobs_root", ""),
        ("modeling_root", "relative"),
        ("blender_executable", ""),
        ("ollama_base_url", "broken"),
    ],
)
...
def test_browse_cancel_preserves_path_and_file_selection(valid_config, monkeypatch):
    dialog = SettingsDialog(valid_config, load_models=False)
    monkeypatch.setattr(
        "forgeflow.ui.dialogs.settings_dialog.QFileDialog.getExistingDirectory", lambda *args: ""
    )
    dialog.browse_path("jobs_root")
    assert dialog.edits["jobs_root"].text() == str(valid_config.jobs_root)
    monkeypatch.setattr(
        "forgeflow.ui.dialogs.settings_dialog.QFileDialog.getOpenFileName",
        lambda *args: (str(valid_config.blender_executable), ""),
    )
