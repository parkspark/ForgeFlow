...
def test_blender_version_increments(config, image):
    ...
    job.blender_requests.append(
        BlenderRequest(
            "x", "C:\\x.glb", str(service.job_directory(job.job_id) / "blender" / "v001"), 1
        )
    )
    service.add_artifact(
        ...
        Artifact(
            "glb",
            str(service.job_directory(job.job_id) / "blender" / "v002" / "x.glb"),
            "blender",
            utc_now(),
            version=2,
        ),
...
def test_artifact_batch_can_join_larger_state_commit(config, image, monkeypatch):
    ...
    artifact = Artifact(
        "glb",
        str(service.job_directory(job.job_id) / "modeling" / "source.glb"),
        "modeling",
        utc_now(),
    )
    writes = 0
