...
from forgeflow.domain.job import UnitySession, UnityTurn, utc_now
...
def test_schema_v2_migrates_to_v3_with_unity_stage(config, image):
    ...
    for key in (
        "unity_project_path", "unity_asset_path", "unity_sessions", "unity_turns",
        "latest_unity_scene_path", "latest_unity_screenshot_path",
    ):
...
def test_command_passes_explicit_project_and_isolated_environment(
    config, unity_project, tmp_path
):
...
def test_project_identity_mismatch_is_blocked(config, image, unity_project, tmp_path):
    ...
    adapter._handle_event(
        {"type": "session_ready", "projectPath": str(tmp_path / "Other Project")}
    )
...
def test_unity_session_and_turn_round_trip(config, image, unity_project):
    ...
    adapter._handle_event(
        {"type": "turn_completed", "id": turn.turn_id, "runLogPath": "run.log"}
    )
    restored = jobs.load(job.job_id)
...
def test_chat_without_fbx_and_effective_prompt_preserves_user_text(
...
def test_selected_fbx_prompt_requires_exact_version_and_job_output_scope(
    ...
    _jobs, job, adapter = _adapter(config, image, unity_project)
    job.unity_asset_path = (
        f"Assets/ForgeFlow/{job.job_id}/Models/v004/Character_humanoid.fbx"
    )
...
def test_repair_prompt_includes_human_feedback(config, image, unity_project):
    jobs, job, adapter = _adapter(config, image, unity_project)
    source = UnityTurn(
        "turn-old", "session-test", "백덤블링을 추가해줘", "effective",
        status="succeeded", human_review_status="rejected",
    )
