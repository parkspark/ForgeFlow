...
def make_adapter(config, image, *, jobs_root: Path | None = None):
    ...
    jobs.add_artifact(job, Artifact("glb", str(source), "modeling", utc_now(), sha256=sha256_file(source)))
...
def add_blender_glb(jobs, job, version: int, payload: bytes = b"edited") -> Path:
    ...
    jobs.add_artifact(job, Artifact("glb", str(path), "blender", utc_now(), version=version,
...
def test_schema_v1_migrates_to_v3_without_losing_fields(config, image):
    ...
    for key in ("rigging_requests", "rigging_input_path", "humanoid_fbx_path", "humanoid_blend_path", "unity_input_path"):
...
def test_direct_execution_and_command_arguments(config, image):
    ...
    assert command.arguments[command.arguments.index("-OutputDirectory") + 1] == str(run.final_directory)
...
def test_space_path_uses_safe_staging(config, image, tmp_path, monkeypatch):
    ...
    jobs.add_artifact(job, Artifact("glb", str(spaced), "blender", utc_now(), version=4,
                                    sha256=sha256_file(spaced)))
...
def test_space_in_staging_root_is_rejected(config, image, tmp_path, monkeypatch):
    ...
    spaced.parent.mkdir()
    spaced.write_bytes(b"space-input")
    jobs.add_artifact(job, Artifact("glb", str(spaced), "blender", utc_now(), version=1,
                                    sha256=sha256_file(spaced)))
...
@pytest.mark.parametrize("mode, kind, message", [("omit", "skin_fbx", "없습니다"), ("empty", "unity_fbx", "0바이트")])
...
def test_restart_recovers_running_rigging_request(config, image):
    ...
    request = RiggingRequest("C:/input.glb", "a" * 64, "C:/output", 1, 12345, "rig_job_v001",
