...
def test_mock_engine_full_non_ui_pipeline(config, image):
    ...
    assert SyncProcessRunner().run(ProcessCommand(sys.executable, ["-c", code], config.jobs_root)) == 0
    ...
    plan = {
        "steps": [{"number": 1, "tool": "asset.set_smooth_shading", "arguments": {
            "input_path": str(source), "output_directory": str(output), "target": "all"
        }, "description": "전체 smooth shading"}], "requires_approval": True,
    }
    proposal = output / "proposal.json"
    proposal.write_text(json.dumps({"plan_sha256": plan_hash(plan), "result": {"status": "denied", "plan": plan}}), encoding="utf-8")
    request = BlenderRequest("전체 smooth shading", str(source), str(output), 1, proposal_path=str(proposal))
    ...
    execution.write_text(json.dumps({"plan_sha256": request.plan_sha256, "result": {"status": "completed", "artifacts": produced}}), encoding="utf-8")
