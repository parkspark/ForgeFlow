...
def test_release_targets_both_models_and_their_servers(config, monkeypatch):
    ...
    assert [c.arguments for c in commands] == [
        ["stop", config.ollama_model],
        ["stop", config.unity_agent_model],
    ]
    assert [c.environment["OLLAMA_HOST"] for c in commands] == [
...
def test_pipeline_waits_for_all_releases_and_stops_on_failure(
    config, monkeypatch, stage, failure_index
):
    ...
    pipeline = SimpleNamespace(
        _begin=begin, log_received=SimpleNamespace(emit=lambda message: None)
    )
    pipeline._release_models = lambda *args: PipelineService._release_models(pipeline, *args)
    pipeline._release_models(
        None, stage, commands, None, lambda: ready.append(True), failures.append
    )
